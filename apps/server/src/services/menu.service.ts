...
export class MenuService {
	...
	async create(data: MenuItemInput): Promise<MenuItemRow> {
		...
		if (!section) throw new HttpError(StatusCodes.BAD_REQUEST, ApiMessage.SECTION_REQUIRED);
		if (!label) throw new HttpError(StatusCodes.BAD_REQUEST, ApiMessage.LABEL_REQUIRED);
		if (!path) throw new HttpError(StatusCodes.BAD_REQUEST, ApiMessage.PATH_FIELD_REQUIRED);
	...
	async update(id: number, data: MenuItemInput): Promise<MenuItemRow | null> {
		return this.prisma.$transaction(async (tx) => {
			...
			if (data.section !== undefined && data.section !== current.section) {
				throw new HttpError(StatusCodes.BAD_REQUEST, ApiMessage.MENU_SECTION_MOVE_UNSUPPORTED);
			}
